...
def ajax_find_peuplate_sequence(request):
    ...
    if keyword :
        bibliotexs  = Bibliotex.objects.filter( title__icontains=keyword,    teacher = request.user.teacher , subjects=subject,levels=level )
    else :
    ...
    if keyword and level_id :
        ...
        bibliotexs  = Bibliotex.objects.filter( title__icontains=keyword, teacher = request.user.teacher , subjects=subject  )
    else :
        level = Level.objects.get(pk=level_id)
        bibliotexs  = Bibliotex.objects.filter(teacher = request.user.teacher , subjects=subject ,levels=level )
