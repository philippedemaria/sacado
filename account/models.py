...
class Student(ModelWithCode):
    ...
    def list_percent_exercise_to_parcours(self):
        ...
        my_list = list()
        percents = self.percents.filter(today=today) 
        is_created = False
    ...
    def this_is_locked(self,relation):
        booleen = False
        if relation.parcours.is_evaluation and self.answers.filter(exercice = relation.exercice, parcours = relation.exercice).count() >= relation.maxexo :
            booleen = True
        return booleen
...
class Teacher(models.Model):
    ...
    levels        = models.ManyToManyField(Level, related_name="levels_to_group", blank=True, verbose_name="Niveaux souhaités")
