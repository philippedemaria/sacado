...
def list_parcours_group(request,id):
    ...
    for student in students :
        if access :
            pcs = Parcours.objects.filter(Q(teacher=teacher)|Q(author=teacher)|Q(coteachers = teacher),students= student, is_favorite=1,level = group.level).exclude(is_leaf=1).order_by("is_evaluation","ranking")
        else :
            pcs = student.students_to_parcours.filter(Q(teacher=teacher)|Q(author=teacher), is_favorite=1,level = group.level).exclude(is_leaf=1).order_by("is_evaluation","ranking")
        for parcours in pcs : 
