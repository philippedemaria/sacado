...
def ajax_find_peuplate_sequence(request):
    ...
    if keyword and level_id :
        ...
        quizzes = Quizz.objects.filter( title__icontains=keyword, teacher = request.user.teacher , subject_id=subject_id,levels=level,is_numeric=1 )
    elif keyword :
        quizzes = Quizz.objects.filter( title__icontains=keyword, teacher = request.user.teacher , subject_id=subject_id, is_numeric=1 )
    else :
        level = Level.objects.get(pk=level_id)
